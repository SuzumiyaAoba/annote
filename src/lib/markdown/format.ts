export function wrapSelection(text: string, from: number, to: number, wrapper: string): { text: string; from: number; to: number } {
...
export function wrapBlock(text: string, from: number, to: number, block: string): { text: string; from: number; to: number } {
  const selected = text.slice(from, to) || "";
